 ...
 * Read in nodes.
 */
List buildNodes(String definitions) {
  List<Node> nodes = new List();

  List<String> items = definitions.split(";");
  ...
  for (var item in items) {
    ...
    if (parts.length > 2) {
      ...
      for (int i = 0; i < count; i++) {
        children.add(parts[2 + i].trim());
      }

      node = new Node(name, weight, children);
    } else {
...
main(List<String> arguments) {
  ...
  // Build a map of all child node names
  Map<String, String> map = new Map();

  for (var node in nodes) {

    if (node.children.length > 0) {
      for (String childName in node.children) {
        map.putIfAbsent(childName, ()=>childName);
      }
    }
  }

  // Now find the node whose name is not present in the children
  // TODO: THere is probably a functional way to do this.
