...
/**
 * Build tree of populated nodes.
 *
 */
Map buildTree(String definitions) {
  ...
  for (var item in items) {
    ...
    if (parts.length > 2) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        String childName = parts[2 + i].trim();

        Node target = map[childName];

        if (target == null) {
          // New Node
          target  = new Node(childName, 0, []);

          // Update map
          map.putIfAbsent(childName, ()=>target);
        }
